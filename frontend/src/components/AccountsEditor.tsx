...
import { api, type Account, type AccountType } from '@/src/lib/api';
...
/** Roll-up of account balances by tax treatment, in today's dollars. */
...
/**
 * Full itemized-accounts manager: an editable balance table, CSV import, and a
 * personal API key for pushing balances from a script. "Pull totals into my plan"
 * hands the rolled-up balances to the parent via {@link onPull} so the profile
 * form's Traditional / Roth / Taxable fields stay in sync (persisted on Save).
 */
export default function AccountsEditor({ onPull }: { onPull?: (rollup: AccountRollup) => void }) {
  ...
  function rollup(): AccountRollup {
    const r: AccountRollup = { traditional: 0, roth: 0, taxable: 0, other: 0, total: 0 };
    for (const a of accounts) {
      const bal = a.balance || 0;
      if (a.type === 'TRADITIONAL') r.traditional += bal;
      else if (a.type === 'ROTH') r.roth += bal;
      else if (a.type === 'TAXABLE') r.taxable += bal;
      else r.other += bal;
      r.total += bal;
    }
    return r;
  ...
  function pullIntoPlan() {
    const r = rollup();
    onPull?.(r);
    setStatus(`Pulled ${money(r.total)} into your plan — Save your info to keep it.`);
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <p className="max-w-xl text-xs opacity-55">
          Keep your balances current here, import a CSV, or push them from a script via the API. Then pull the
          totals into your plan to set your Traditional / Roth / Taxable amounts.
        </p>
              ...
              <tr key={i} className="border-b border-black/5 last:border-0 dark:border-white/5">
                ...
                <td className="p-2">
                  <div className="flex items-center">
                    ...
                    <NumericInput value={a.balance} step={1000} ariaLabel="Balance"
                      onChange={(v) => patch(i, { balance: v })}
      ...
      <div className="flex flex-wrap items-center gap-3">
        <button type="button" onClick={addRow} className="rounded-md border border-black/15 px-3 py-2 text-sm dark:border-white/15">+ Add account</button>
        ...
        <button type="button" onClick={save} className="rounded-md px-4 py-2 text-sm font-medium"
          style={{ background: 'var(--foreground)', color: 'var(--background)' }}>Save accounts</button>
        <button type="button" onClick={pullIntoPlan} className="rounded-md border border-cyan-500/50 px-4 py-2 text-sm text-cyan-700 dark:text-cyan-400">
      ...
      <div className="rounded-lg border border-black/10 p-4 dark:border-white/10">
        <div className="flex items-center justify-between gap-3">
          ...
          <button type="button" onClick={() => api.generateApiKey().then((r) => setApiKey(r.apiKey)).catch(() => {})}
